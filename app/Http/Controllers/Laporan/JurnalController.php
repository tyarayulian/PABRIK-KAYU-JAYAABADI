...
class JurnalController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('report.journal', compact('journals', 'totalDebit', 'totalCredit', 'year', 'month', 'years', 'months', 'filterType', 'startDate', 'endDate', 'search'));
    }
        ...
        $callback = function () use ($journals, $filterType, $month, $year, $startDate, $endDate) {
            ...
            foreach ($journals as $j) {
                ...
                fputcsv($file, [
                    ...
                    $j->account->code ?? '-',
                    $j->account->name ?? '-',
                    $isNewTransaction ? ($j->reference ?? ('#TX-'.str_pad($j->source_id ?? $j->id, 5, '0', STR_PAD_LEFT))) : '',
                    $debit ?: 0,
                    $credit ?: 0,
                ]);
