...
<?php $__env->startSection('breadcrumb', 'Transaksi Kas'); ?>
    ...
    <div class="stat-cards-grid" style="margin-top: 30px;">
        <div class="stat-card">
            ...
            <div class="stat-info">
                ...
                <div class="stat-card-value" id="statIncomeValue">Rp<?php echo e(number_format($allTransactions->where('type', 'income')->sum('amount'), 0, ',', '.')); ?></div>
                <div class="stat-card-meta" id="statIncomeCount"><?php echo e($allTransactions->where('type', 'income')->count()); ?> transaksi</div>
            </div>
        </div>
        <div class="stat-card">
            ...
            <div class="stat-info">
                ...
                <div class="stat-card-value" id="statExpenseValue">Rp<?php echo e(number_format($allTransactions->where('type', 'expense')->sum('amount'), 0, ',', '.')); ?></div>
                <div class="stat-card-meta" id="statExpenseCount"><?php echo e($allTransactions->where('type', 'expense')->count()); ?> transaksi</div>
            </div>
        </div>
        <div class="stat-card">
            ...
            <div class="stat-info">
                ...
                <div class="stat-card-value" id="statNetValue">Rp<?php echo e(number_format($allTransactions->where('type', 'income')->sum('amount') - $allTransactions->where('type', 'expense')->sum('amount'), 0, ',', '.')); ?></div>
    ...
    <div class="table-section" id="combinedTableSection">
        ...
        <div class="table-title-section">
            ...
            <div style="display: flex; gap: 12px; align-items: center;">
                <a href="<?php echo e(route('cash.in.create')); ?>" class="btn-action btn-action-primary" style="background: #f0f4ff; color: #1e2a78; border: 1px solid #e2e8f0; box-shadow: none;">
                    <i class="fas fa-plus"></i> Pemasukan
                </a>
                <a href="<?php echo e(route('cash.out.create')); ?>" class="btn-action btn-action-primary" style="background: #fff1f2; color: #f43f5e; border: 1px solid #ffe4e6; box-shadow: none;">
        ...
        <div id="filterPanel" class="filter-row">
            ...
            <div class="filter-actions">
                ...
                <a href="<?php echo e(route('cash.index')); ?>" class="btn-reset-filter">
                    Reset
                </a>
            </div>
        ...
        <div style="padding: 0;">
            <?php $__empty_1 = true; $__currentLoopData = $allTransactions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $transaction): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div data-transaction-id="<?php echo e($transaction['id']); ?>" data-transaction-type="<?php echo e($transaction['type']); ?>" data-amount="Rp<?php echo e(number_format($transaction['amount'], 0, ',', '.')); ?>" class="transaction-row">
                    <!-- NO COLUMN -->
                    ...
                    <div style="text-align: left;">
                        <div style="font-size: 14px; color: #1e293b; font-weight: 600;">
                            <?php echo e(\Carbon\Carbon::parse($transaction['date'])->locale('id')->translatedFormat('d M Y')); ?>

                        </div>
                        <div style="font-size: 12px; color: #94a3b8; margin-top: 2px;"><?php echo e(\Carbon\Carbon::parse($transaction['date'])->format('H:i')); ?></div>
                    </div>

                    <!-- CATEGORY -->
                    <div style="text-align: left;">
                        <div style="font-size: 14px; color: #1e293b; font-weight: 600;">
                            <?php echo e($transaction['category']->name ?? 'N/A'); ?>
                    ...
                    <div style="text-align: left;">
                        <?php if($transaction['file_path']): ?>
                            <div style="position: relative; display: inline-block;">
                                ...
                                ?>
                                <?php if($isImage): ?>
                                    <img src="<?php echo e($fileUrl); ?>" style="width: 36px; height: 36px; border-radius: 8px; object-fit: cover; cursor: pointer; border: 1px solid #e2e8f0;" onclick="openViewer('<?php echo e($fileUrl); ?>', '<?php echo e($fileName); ?>')">
                                <?php else: ?>
                                    <div style="width: 36px; height: 36px; border-radius: 8px; background: #f1f5f9; display: flex; align-items: center; justify-content: center; cursor: pointer;" onclick="openViewer('<?php echo e($fileUrl); ?>', '<?php echo e($fileName); ?>')">
                    ...
                    <div style="text-align: left; display: flex; gap: 8px; align-items: center;">
                        <button title="Lihat Detail" style="background: #f1f5f9; color: #1e293b; border: 1px solid #e2e8f0; width: 34px; height: 34px; border-radius: 10px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s;" onclick="showTransactionDetail(<?php echo e($transaction['id']); ?>, '<?php echo e($transaction['type']); ?>')"><i class="fas fa-eye"></i></button>
                        
                        <button title="Edit Transaksi" style="background: #f8fafc; color: #1e2a78; border: 1px solid #e2e8f0; width: 34px; height: 34px; border-radius: 10px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s;" onclick="editTransaction('<?php echo e($transaction['type']); ?>', <?php echo e($transaction['id']); ?>)"><i class="fas fa-edit"></i></button>
                        
                        <button title="Hapus Transaksi" style="background: #fff1f2; color: #f43f5e; border: 1px solid #ffe4e6; width: 34px; height: 34px; border-radius: 10px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s;" onclick="deleteTransaction('<?php echo e($transaction['type']); ?>', <?php echo e($transaction['id']); ?>)"><i class="fas fa-trash"></i></button>
                    </div>
                </div>
    ...
    <div class="modal-content">
        ...
        <form id="transactionForm" enctype="multipart/form-data">
            ...
            <div style="margin-bottom: 20px;">
                ...
                <input type="date" name="date" id="txDate" required value="<?php echo e(date('Y-m-d')); ?>" style="width: 100%; padding: 12px 16px; border: 1px solid #e2e8f0; border-radius: 12px; font-size: 15px; box-sizing: border-box; background: #f8fafc;">
            </div>
...
<script>
    ...
    const akunMasuk = <?php echo json_encode($akunMasuk, 15, 512) ?>;
    const akunKeluar = <?php echo json_encode($akunKeluar, 15, 512) ?>;
    ...
    function openModal(type, id = null) {
        ...
        if (id) {
            ...
            form.reset();
            document.getElementById('txDate').value = '<?php echo e(date('Y-m-d')); ?>';
            form.action = type === 'income' ? '<?php echo e(route('cash.in.store')); ?>' : '<?php echo e(route('cash.out.store')); ?>';
        }
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\xampp4\htdocs\pabrik-kayu1\resources\views/transaksi-kas/index.blade.php ENDPATH**/ ?>