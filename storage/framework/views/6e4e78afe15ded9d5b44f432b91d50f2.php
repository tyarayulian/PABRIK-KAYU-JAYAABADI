...
<?php $__env->startSection('breadcrumb', 'Transaksi > Penjualan > Edit'); ?>
...
<div class="page-container">
    <div class="card">
        ...
        <form action="<?php echo e(route('kas-masuk.update', $kasMasuk->id)); ?>" method="POST" enctype="multipart/form-data">
            <?php echo csrf_field(); ?>
                ...
                <div class="form-section">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="date" name="date" value="<?php echo e(old('date', $kasMasuk->date->format('Y-m-d'))); ?>" class="form-control">
                        </div>

                        <div class="form-group">
                            <label>Kategori Pemasukan*</label>
                            <select id="category_id_main" class="form-control" onchange="handleMainCategoryChange(this)">
                                <option value="">Pilih Kategori</option>
                                <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($cat->id); ?>" data-is-product="<?php echo e($cat->is_product); ?>" <?php echo e(old('category_id', $kasMasuk->category_id) == $cat->id ? 'selected' : ''); ?>>
                                        <?php echo e($cat->name); ?>
                        ...
                        <div id="product-group" class="form-group full-width" style="display: <?php echo e($kasMasuk->product_id ? 'block' : 'none'); ?>;">
                            <label>Produk*</label>
                            <select id="product_id" name="product_id" class="form-control" onchange="handleProductChange(this)">
                                ...
                                <?php $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($product->id); ?>" data-category-id="<?php echo e($product->category_id); ?>" data-price="<?php echo e((int)$product->price); ?>" data-unit="<?php echo e($product->unit); ?>" data-stock="<?php echo e((float)$product->stock); ?>" <?php echo e(old('product_id', $kasMasuk->product_id) == $product->id ? 'selected' : ''); ?>><?php echo e($product->name); ?></option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                            <div id="stock-info" style="display: <?php echo e($kasMasuk->product_id ? 'block' : 'none'); ?>; font-size: 11px; color: #1e2a78; margin-top: 6px; font-weight: 700;">
                                Sisa stok: <span id="stock-value"><?php echo e($kasMasuk->product->stock ?? 0); ?></span> <span id="stock-unit"><?php echo e($kasMasuk->product->unit ?? ''); ?></span>
                            </div>
                        </div>

                        <input type="hidden" id="category_id" name="category_id" value="<?php echo e(old('category_id', $kasMasuk->category_id)); ?>">
                        <input type="hidden" id="account_id" name="account_id" value="<?php echo e(old('account_id', $kasMasuk->account_id)); ?>">

                        <div id="qty-price-group" class="form-group full-width" style="display: <?php echo e($kasMasuk->product_id ? 'block' : 'none'); ?>;">
                            <div class="form-grid" style="grid-template-columns: 1fr 1fr; gap: 20px;">
                                <div>
                                    <label>QTY <span id="unit-label" style="text-transform: none; color: #64748b;"><?php echo e($kasMasuk->product->unit ? '('.$kasMasuk->product->unit.')' : ''); ?></span>*</label>
                                    <input type="number" step="1" id="quantity" name="quantity" placeholder="0" value="<?php echo e(old('quantity', (int)$kasMasuk->quantity)); ?>" class="form-control" oninput="calculateAmount()">
                                </div>
                                <div>
                                    <label>Harga Satuan (RP)*</label>
                                    <input type="text" id="price" name="price" placeholder="Rp 0" value="<?php echo e(old('price', (int)$kasMasuk->price)); ?>" class="form-control" oninput="calculateAmount()">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group full-width">
                            ...
                            <input type="text" id="amount" name="amount" placeholder="Rp 0" value="<?php echo e(old('amount', (int)$kasMasuk->amount)); ?>" class="form-control" <?php echo e($kasMasuk->product_id ? 'readOnly' : ''); ?> style="background-color: <?php echo e($kasMasuk->product_id ? '#f1f5f9' : '#f8fafc'); ?>">
                        </div>

                        <div class="form-group full-width">
                            <label>Keterangan / Deskripsi</label>
                            <textarea name="description" placeholder="Misal: Pembayaran piutang dari Customer A" class="form-control" rows="3" style="resize: none;"><?php echo e(old('description', $kasMasuk->description)); ?></textarea>
                        </div>
                    </div>
                </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\xampp4\htdocs\pabrik-kayu1\resources\views/transaksi/kas-masuk/edit.blade.php ENDPATH**/ ?>