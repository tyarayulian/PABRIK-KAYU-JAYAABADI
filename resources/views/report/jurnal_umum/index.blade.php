    ...
    <div class="table-section">
                ...
                @forelse($journals as $group)
                    @foreach($group['entries'] as $entry)
                        <tr>
                            ...
                            {{-- Ref = kode akun lawan (counter account code) per baris --}}
        ...
        @if($journals->hasPages())
            <div class="pagination-container" style="padding: 20px 24px; border-top: 1px solid #f1f5f9; display: flex; align-items: center; justify-content: center; gap: 4px;">
                ...
                @else
                    <a href="{{ $journals->previousPageUrl() }}" style="padding: 8px 12px; border: 1px solid #e2e8f0; border-radius: 8px; color: #475569; font-size: 13px; font-weight: 600; text-decoration: none;">‹</a>
                @endif

                {{-- Pages --}}
                @foreach($journals->getUrlRange(1, $journals->lastPage()) as $page => $url)
                    @if($page == $journals->currentPage())
                        <span style="padding: 8px 14px; background: #1e2a78; color: white; border-radius: 8px; font-size: 13px; font-weight: 700;">{{ $page }}</span>
                    @else
                        <a href="{{ $url }}" style="padding: 8px 14px; border: 1px solid #e2e8f0; border-radius: 8px; color: #475569; font-size: 13px; font-weight: 600; text-decoration: none;">{{ $page }}</a>
                    @endif
                @endforeach

                {{-- Next --}}
                @if($journals->hasMorePages())
                    <a href="{{ $journals->nextPageUrl() }}" style="padding: 8px 12px; border: 1px solid #e2e8f0; border-radius: 8px; color: #475569; font-size: 13px; font-weight: 600; text-decoration: none;">›</a>
                @else
                    <span style="padding: 8px 12px; border: 1px solid #e2e8f0; border-radius: 8px; color: #cbd5e1; font-size: 13px; font-weight: 600; cursor: not-allowed;">›</span>
                @endif
            </div>
