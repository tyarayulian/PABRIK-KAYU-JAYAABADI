...
<div class="page-container">
    <div class="card">
        <div class="card-header">
            ...
            <a href="{{ route('sales-return.create') }}" class="btn-cancel-header">
                <i class="fas fa-times"></i> Batal
            </a>
        </div>

        <form action="{{ route('sales-return.store') }}" method="POST" onsubmit="return validateBeforeSubmit()">
            @csrf
                    ...
                    <div>
                        <div class="info-box">
                            <div class="info-box-left">
                                ...
                                <div class="info-box-value">{{ $sale->product ? $sale->product->name : ($sale->category->name ?? 'Penjualan Umum') }}</div>
                                <div class="info-box-sub">{{ $sale->date->format('d M Y') }} &nbsp;|&nbsp; Total: Rp {{ number_format($sale->amount, 0, ',', '.') }}</div>
                            </div>
                            <div class="info-box-right">
                                <div class="info-box-label">Maksimal Retur</div>
                                <div class="info-box-value">{{ number_format($maxQty, 0, ',', '.') }} {{ $sale->product->unit ?? 'Kubik' }}</div>
                            </div>
                        </div>
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" step="1" min="1" max="{{ $maxQty }}" id="quantity" name="quantity"
                                    placeholder="0" value="{{ old('quantity') }}" class="form-control"
                                    oninput="calculateAmount()" required>
                                <small style="color: #94a3b8; margin-top: 6px; display: block; font-weight: 500;">
                                    Maks: {{ $maxQty }} {{ $sale->product->unit ?? 'kubik' }}
                                </small>
                            </div>
                            ...
                            <div class="form-group full-width">
                                ...
                                <small style="color: #94a3b8; margin-top: 8px; display: block; font-weight: 500;">
                                    Otomatis: QTY × Harga Jual (Rp {{ number_format($sale->price, 0, ',', '.') }}/kubik)
                                </small>
                            </div>
                            ...
                            <div class="form-group full-width">
                                ...
                                <textarea name="description" placeholder="Jelaskan alasan pengembalian (contoh: barang cacat, tidak sesuai pesanan)..."
                                    class="form-control" rows="3" style="resize: none;">{{ old('description') }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
