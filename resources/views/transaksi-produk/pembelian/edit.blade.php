...
@section('breadcrumb', 'Transaksi > Pembelian > Edit')
...
<style>
    ...
    input.form-control[readonly] { background-color: #f1f5f9; color: #64748b; cursor: default; }
    select.form-control { cursor: pointer; background-color: #f8fafc; color: #1e293b; }
    select.form-control option { color: #1e293b; }
                ...
                <div class="form-section">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="date" name="date" value="{{ old('date', $kasKeluar->date->format('Y-m-d')) }}" class="form-control">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select id="product_id" name="product_id" class="form-control" onchange="handleProductChange(this)">
                                ...
                                @foreach($products as $product)
                                    <option value="{{ $product->id }}" data-category-id="{{ $product->category_id }}" data-cost="{{ (int)$product->cost }}" data-unit="{{ $product->unit }}" data-stock="{{ (float)$product->stock }}" {{ old('product_id', $kasKeluar->product_id) == $product->id ? 'selected' : '' }}>{{ $product->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group full-width">
                            ...
                            <input type="text" name="hutan" placeholder="Contoh: Kalimantan, Jepara, Hutan Produksi Riau" value="{{ old('hutan', $kasKeluar->hutan) }}" class="form-control">
                        </div>

                        <input type="hidden" id="category_id" name="category_id" value="{{ old('category_id', $kasKeluar->category_id) }}">
                        <input type="hidden" id="account_id" name="account_id" value="{{ old('account_id', $kasKeluar->account_id) }}">

                        <div class="form-group">
                            <label>QTY <span id="unit-label" style="text-transform: none; color: #64748b;">{{ $kasKeluar->product->unit ? '('.$kasKeluar->product->unit.')' : '' }}</span>*</label>
                            <input type="number" step="1" id="quantity" name="quantity" placeholder="0" value="{{ old('quantity', (int)$kasKeluar->quantity) }}" class="form-control" oninput="calculateAmount()">
                        </div>

                        <div class="form-group">
                            <label>Harga Satuan (RP)*</label>
                            <input type="text" id="price" name="price" placeholder="Rp 0" value="{{ old('price', (int)$kasKeluar->price) }}" class="form-control" oninput="calculateAmount()">
                        </div>

                        <div class="form-group full-width">
                            <label>Jumlah Nominal (RP)*</label>
                            <input type="text" id="amount" name="amount" placeholder="Rp 0" value="{{ old('amount', (int)$kasKeluar->amount) }}" class="form-control" readOnly style="background-color: #f1f5f9;">
                        </div>

                        <div class="form-group full-width">
                            <label>Keterangan / Deskripsi</label>
                            <textarea name="description" placeholder="Misal: Pembelian bahan baku dari UD. Rimba" class="form-control" rows="3" style="resize: none;">{{ old('description', $kasKeluar->description) }}</textarea>
                        </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Hitung ulang amount dari qty × price yang sudah diformat
        calculateAmount();
