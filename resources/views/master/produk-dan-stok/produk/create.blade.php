...
@section('breadcrumb', 'Master Data > Produk > Tambah')
...
<style>
    ...
    .size-row { display: grid; grid-template-columns: 2fr 2fr 2fr 40px; gap: 15px; margin-bottom: 20px; align-items: flex-end; }
...
<div class="page-container">
    <div class="card">
        ...
        <form action="{{ route('master.products.store') }}" method="POST" id="productForm">
            @csrf
            
            <div class="card-body">
                <div id="validationErrors" style="display: none; background: #fff1f2; color: #dc2626; padding: 16px; border-radius: 12px; margin-bottom: 32px; font-weight: 600; border: 1px solid #ffe4e6;">
                </div>
                ...
                <div class="form-section">
                    <div class="section-info">
                        ...
                        <p>Daftarkan produk baru ke dalam sistem.</p>
                    </div>
                    <div>
                        <div class="form-group" style="margin-bottom: 30px;">
                            <label>Jenis Kayu*</label>
                            <input type="text" name="wood_type" id="wood_type" class="form-control" placeholder="Contoh: Duren, Senggon, Racuk" value="{{ request('wood_type') }}" required {{ request('wood_type') ? 'readonly' : '' }}>
                        </div>
                        <div class="form-group">
                            <label>Kategori Bentuk (Pilih Checklist)*</label>
                            <div class="category-checklist">
                                @foreach(['Balok', 'Papan', 'Kasau'] as $cat)
                                <div class="check-item">
                                    <input type="checkbox" id="check-{{ $cat }}" class="cat-checkbox" data-category="{{ $cat }}">
                                    <label for="check-{{ $cat }}" class="check-label">
                                        <i class="fas fa-check-circle"></i> {{ $cat }}
                                    </label>
                                </div>
                                @endforeach
                            </div>
                            <div id="category-error" style="display: none; color: #ef4444; font-size: 12px; font-weight: 600; margin-top: 8px;">
                                Mohon pilih minimal 1 kategori bentuk (Balok, Papan, atau Kasau)
                            </div>
                        </div>
                    </div>
                </div>
...
<template id="size-row-template">
    <div class="size-row">
        ...
        <div class="form-group">
            ...
            <input type="number" step="0.01" min="0" name="products[CAT_IDX][items][ROW_IDX][stock]" class="form-control stock-input" value="0" required>
        </div>
        <div class="form-group">
            <label>HPP / KUBIK*</label>
            <input type="text" name="products[CAT_IDX][items][ROW_IDX][cost]" class="form-control hpp-input" placeholder="Rp 0" inputmode="numeric" pattern="[0-9.]*" required>
        </div>
...
<script>
    ...
    // Validation for Jenis Kayu field
    const woodTypeInput = document.getElementById('wood_type');
    if (woodTypeInput && !woodTypeInput.hasAttribute('readonly')) {
        woodTypeInput.addEventListener('invalid', function() {
            this.setCustomValidity('Jenis Kayu wajib diisi (contoh: Duren, Senggon, Racuk)');
        });
        woodTypeInput.addEventListener('input', function() {
    ...
    function addSizeRow(cat, catIdx) {
        ...
        // Add validation for Ukuran (Size) input
        const sizeInput = newRow.querySelector('input[name*="[size]"]');
        if (sizeInput) {
            sizeInput.addEventListener('invalid', function() {
                this.setCustomValidity('Ukuran wajib diisi (contoh: 8x12x4)');
            });
        ...
        // Add validation for QTY input
        const qtyInput = newRow.querySelector('.stock-input');
        if (qtyInput) {
            qtyInput.addEventListener('invalid', function() {
                this.setCustomValidity('QTY harus diisi dengan angka yang valid (contoh: 10 atau 10.5)');
            });
        ...
        // Add formatting for HPP input
        const hppInput = newRow.querySelector('.hpp-input');
        if (hppInput) {
            // Format number with thousand separators and Rp prefix
            ...
            hppInput.addEventListener('keypress', function (e) {
            ...
            // Handle paste event
            hppInput.addEventListener('paste', function (e) {
                e.preventDefault();
            ...
            hppInput.addEventListener('invalid', function() {
                this.setCustomValidity('HPP harus diisi dengan angka yang valid (contoh: Rp 1.000.000)');
            });
        }
    }
    ...
    document.getElementById('productForm').addEventListener('submit', function(e) {
        ...
        const checkedCategories = document.querySelectorAll('.cat-checkbox:checked');
        ...
        // Convert formatted HPP values to plain numbers before submit
        const hppInputs = this.querySelectorAll('.hpp-input');
    ...
    // Hide error when user selects a category
    document.querySelectorAll('.cat-checkbox').forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            const categoryError = document.getElementById('category-error');
            const checkedCategories = document.querySelectorAll('.cat-checkbox:checked');
