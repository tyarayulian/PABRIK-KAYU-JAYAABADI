...
@section('title', 'Edit Stok Awal: ' . $product->name)
@section('breadcrumb', 'Master Data > Produk > Edit Stok Awal')
...
<style>
    ...
    .form-group label { 
        display: block; margin-bottom: 10px; font-weight: 800; 
        color: #334155; font-size: 12px; text-transform: uppercase;
    }
    .form-control { 
        width: 100%; padding: 14px 16px; border: 1.5px solid #f1f5f9; 
        border-radius: 12px; font-size: 14px; font-weight: 500; color: #1e293b;
        background-color: #f8fafc; transition: all 0.2s ease; box-sizing: border-box;
    }
    .form-control:focus {
        outline: none; border-color: #1e2a78;
        background-color: #ffffff; box-shadow: 0 0 0 4px rgba(30, 42, 120, 0.05);
    }
    .card-footer {
    ...
    .btn-save { 
        background: #1e2a78; color: white; border: none; padding: 14px 32px;
...
<div class="page-container">
    <div class="card">
        <div class="card-header">
            <div>
                <div style="font-size: 11px; font-weight: 700; color: #94a3b8; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 6px;">Edit Stok Awal</div>
                <h2>{{ $product->name }}</h2>
            </div>
        ...
        <form action="{{ route('master.products.update-initial-stock', $product->id) }}" method="POST" onsubmit="return stripFormattingBeforeSubmit(this)">
            @csrf
            @method('PUT')
            ...
            <div class="card-body">
                ...
                <div class="form-section">
                    <div class="section-info">
                        <h3>Detail Stok Awal</h3>
                        <p>Ubah jumlah stok awal dan HPP untuk produk ini.</p>
                    </div>
                    <div class="form-grid">
                        <div class="form-group">
                            <label>QTY / Kubik*</label>
                            <input type="text" inputmode="decimal" name="quantity" class="form-control"
                                   value="{{ old('quantity', (float)$product->initial_stock) }}"
                                   placeholder="0" required autofocus>
                        </div>
                        <div class="form-group">
                            <label>Harga Pembelian (HPP)*</label>
                            <input type="text" name="price" id="price" class="form-control"
                                   value="{{ old('price', (int)$product->cost) }}"
            ...
            <div class="card-footer">
                <a href="{{ route('master.products.index') }}" class="btn-cancel-header" style="padding: 14px 24px;">Batal</a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function formatCurrency(val) {
            ...
            if (!value) return '';
            return 'Rp ' + new Intl.NumberFormat('id-ID').format(parseInt(value));
        }
