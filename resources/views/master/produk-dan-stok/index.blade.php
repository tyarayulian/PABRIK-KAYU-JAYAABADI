...
@section('breadcrumb', 'Master Data > Stok Produk')
...
<style>
    * { font-family: 'Plus Jakarta Sans', sans-serif; }
    ...
    .wood-sidebar-title::after { display: none; }
    ...
    /* Icon buttons di pojok kanan atas product card */
    ...
    .btn-icon-add:hover { background: #f0fdf4; border-color: #16a34a; color: #15803d; transform: translateY(-1px); }

    .btn-icon-edit {
        border-color: #bfdbfe;
        color: #2563eb;
    }
    .btn-icon-edit:hover { background: #eff6ff; border-color: #2563eb; color: #1d4ed8; transform: translateY(-1px); }

    .btn-icon-delete {
        border-color: #fecaca;
        color: #ef4444;
    }
    .btn-icon-delete:hover { background: #fff1f2; border-color: #ef4444; color: #dc2626; transform: translateY(-1px); }

    /* Icon edit/hapus inline di setiap item jenis kayu */
    .wood-item-actions {
        display: none;
        gap: 4px;
        align-items: center;
    ...
    .wood-nav-item:hover .wood-item-actions,
    .wood-nav-item.active .wood-item-actions {
        display: flex;
    }

    .wood-nav-item:hover .wood-chevron,
    .wood-nav-item.active .wood-chevron {
        display: none;
    }
    ...
    .wood-action-edit:hover { background: #eff6ff; }

    .wood-action-delete {
        border-color: #fecaca;
        color: #ef4444;
    }
    .wood-action-delete:hover { background: #fff1f2; }

    /* Active state override - putih agar kontras di background biru */
    .wood-nav-item.active .wood-action-edit {
        border-color: rgba(255,255,255,0.4);
        color: white;
        background: rgba(255,255,255,0.15);
    }
    .wood-nav-item.active .wood-action-edit:hover { background: rgba(255,255,255,0.25); }

    .wood-nav-item.active .wood-action-delete {
        border-color: rgba(255,150,150,0.5);
        color: #fca5a5;
        background: rgba(255,255,255,0.1);
    }
    .wood-nav-item.active .wood-action-delete:hover { background: rgba(255,100,100,0.2); }
...
<div class="stok-layout">
    ...
    <div class="wood-sidebar">
        <div class="wood-sidebar-title">
            <span>Jenis Kayu</span>
            <div style="display: flex; gap: 4px;">
                <a href="{{ route('master.products.create') }}" class="btn-icon btn-icon-add" title="Tambah Produk Baru" style="width: 24px; height: 24px; font-size: 10px; border-radius: 6px;">
                    <i class="fas fa-plus"></i>
                </a>
            </div>
        </div>
        <div class="wood-nav">
            @forelse($groupedProducts as $woodType => $categories)
            @php $firstProd = $categories->flatten()->first(); @endphp
            <div class="wood-nav-item {{ $loop->first ? 'active' : '' }}" 
                 data-wood-name="{{ strtoupper($woodType ?: 'Jenis Kayu Lain') }}"
                 onclick="switchWood(this, 'wood-{{ \Str::slug($woodType ?: 'lain-lain') }}')">
                <span style="flex: 1;">{{ $woodType ?: 'Jenis Kayu Lain' }}</span>
                <div class="wood-item-actions" onclick="event.stopPropagation()">
                    @if($firstProd)
                    <a href="{{ route('master.products.edit', $firstProd->id) }}" class="wood-action-btn wood-action-edit" title="Edit">
                        <i class="fas fa-edit"></i>
                    </a>
                    <button type="button" class="wood-action-btn wood-action-delete" title="Hapus"
                            onclick="confirmDeleteHistory('{{ route('master.products.destroy', $firstProd->id) }}', 'initial')">
                        <i class="fas fa-trash"></i>
                    </button>
                    @endif
                </div>
                <i class="fas fa-chevron-right wood-chevron" style="font-size: 10px; opacity: 0.5;"></i>
            </div>
            @empty
    ...
    <div class="stok-main">
        <div class="header-card">
            <div class="header-content">
                <h1>STOK PRODUK</h1>
                <h2 id="active-wood-name">PABRIK KAYU JAYA ABADI</h2>
                <p>PERIODE : {{ $startDate && $endDate ? \Carbon\Carbon::parse($startDate)->format('d/m/Y') . ' - ' . \Carbon\Carbon::parse($endDate)->format('d/m/Y') : 'BULAN INI' }}</p>
            </div>
            <div class="filter-section">
                <form action="{{ route('master.products.index') }}" method="GET" class="filter-grid">
                    <div class="filter-group">
                        <label>MULAI</label>
                        <input type="date" name="start_date" class="form-control-custom" value="{{ $startDate }}">
                    </div>
                    <div class="filter-group">
                        <label>AKHIR</label>
                        <input type="date" name="end_date" class="form-control-custom" value="{{ $endDate }}">
                    </div>
                    <button type="submit" class="btn-apply">
                        <i class="fas fa-filter"></i> Terapkan
                    </button>
                    <a href="{{ route('master.products.index') }}" class="btn-reset">Reset</a>
                </form>
            </div>
        </div>

        @forelse($groupedProducts as $woodType => $categories)
        <div id="wood-{{ \Str::slug($woodType ?: 'lain-lain') }}" class="wood-content {{ $loop->first ? 'active' : '' }}">
            
            <!-- Tab Kategori -->
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
                <div class="category-tabs" style="margin-bottom: 0;">
                    @foreach($categories as $categoryName => $products)
                    <div class="category-tab-item {{ $loop->first ? 'active' : '' }}" 
                         onclick="switchCategory(this, 'cat-{{ \Str::slug(($woodType ?: 'lain-lain') . '-' . ($categoryName ?: 'umum')) }}')">
                        {{ strtoupper($categoryName ?: 'Umum') }}
                    </div>
                    @endforeach
                </div>
            </div>
            ...
            <!-- Konten per Kategori -->
            @foreach($categories as $categoryName => $products)
            <div id="cat-{{ \Str::slug(($woodType ?: 'lain-lain') . '-' . ($categoryName ?: 'umum')) }}" 
                 class="cat-content {{ $loop->first ? 'active' : '' }}">
                
                @foreach($products->groupBy('size') as $size => $sizeGroup)
                @php
                    ...
                    $returnsHistory = $allHistory->where('type', 'return')->values();
                    ...
                    foreach ($outflows as $o) {
                        $outflowQueue[] = [
                            ...
                            'description' => $o->description ?? 'Penjualan Stok',
                            'type' => 'sale',
                        ];
                    }
                    ...
                    foreach ($inflows as $inflow) {
                        ...
                        // Tambahkan semua retur ke soldItems batch ini
                        foreach ($returnsHistory as $ret) {
                            $soldTo[] = [
                                'date' => $ret->date,
                                'quantity' => abs((float) $ret->quantity),
                                'description' => $ret->description ?? 'Retur Penjualan',
                                'type' => 'return',
                            ];
                            $remaining += abs((float) $ret->quantity); // stok kembali
                        }
                ...
                @endphp
                <div class="product-card" id="product-{{ $firstProduct->id }}">
                    <div class="product-card-header">
                        <div class="product-title-group">
                            <div>
                                <div style="font-size: 11px; font-weight: 700; color: #94a3b8; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 4px;">
                                    {{ $woodType }} &rsaquo; {{ $categoryName }}
                                </div>
                                <h2 style="font-size: 1.5rem; color: #1e2a78; font-weight: 800; margin: 0;">{{ $size ?: $firstProduct->name }}</h2>
                            </div>
                            <span style="background: #eef2ff; padding: 4px 12px; border-radius: 6px; font-size: 11px; color: #4338ca; font-weight: 700;">{{ strtoupper($firstProduct->unit) }}</span>
                        </div>
                        <div style="display: flex; gap: 12px; align-items: center;">
                            <a href="{{ route('master.products.add-stock', $firstProduct->id) }}" class="btn-tambah-stok">
                            ...
                            <tbody>
                                @php
                                    ...
                                    foreach($allHistory as $history) {
                                        if ($history->type != 'sale' && $history->type != 'return') {
                                ...
                                @forelse($processedHistory as $history)
                                    @php
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div style="font-weight: 700; color: {{ $labelColor }}; text-transform: uppercase; font-size: 12px; letter-spacing: 0.5px;">
                                                {{ $labelText }}
                                            </div>
                                        </td>
                                        ...
                                        <td style="text-align: right;" class="mono">
                                            <span style="color: #10b981">
                                                +{{ number_format($history->quantity, fmod($history->quantity, 1) == 0 ? 0 : 2, ',', '.') }}
                                            </span>
                                        </td>
                                        <td style="text-align: right; color: #1e2a78; font-weight: 800;" class="mono">
                                            {{ number_format($remStock, fmod($remStock, 1) == 0 ? 0 : 2, ',', '.') }}
                                        </td>
                                            ...
                                            <div style="display: flex; gap: 8px; justify-content: center;">
                                                ...
                                                @php
                                                    ...
                                                    if ($history->type == 'initial') {
                                                        $editUrl = route('master.products.edit-initial-stock', $history->id);
                                        ...
                                        <td colspan="6" style="padding: 0; background: #f9fbff;">
                                            <div class="detail-container" style="padding: 24px 40px; border-left: 5px solid #1e2a78;">
                                                <div style="font-weight: 800; color: #1e2a78; font-size: 12px; text-transform: uppercase; margin-bottom: 20px; display: flex; align-items: center; gap: 10px;">
                                                    <i class="fas fa-history"></i> History Transaksi dari Batch Ini
                                                </div>
                                                        ...
                                                        <table style="width: 100%; border: none;">
                                                            ...
                                                            <tbody>
                                                                ...
                                                                <tr style="background: transparent;">
                                                                    <td style="padding: 12px 20px; border-bottom: 1px solid #f1f5f9; background: transparent; font-weight: 600; color: #334155;">{{ \Carbon\Carbon::parse($sold['date'])->format('d/m/Y H:i') }}</td>
                                                                    <td style="padding: 12px 20px; border-bottom: 1px solid #f1f5f9; background: transparent; color: #475569;">{{ $sold['description'] }}</td>
                                                                    <td style="padding: 12px 20px; border-bottom: 1px solid #f1f5f9; background: transparent; text-align: right; color: {{ isset($sold['type']) && $sold['type'] === 'return' ? '#10b981' : '#ef4444' }}; font-weight: 800;">
                                                                        @if(isset($sold['type']) && $sold['type'] === 'return')
                                                                            +{{ number_format($sold['quantity'], fmod($sold['quantity'], 1) == 0 ? 0 : 2, ',', '.') }}
                                                                        @else
                                                                            -{{ number_format($sold['quantity'], fmod($sold['quantity'], 1) == 0 ? 0 : 2, ',', '.') }}
                                                                        @endif
                                                                    </td>
                    ...
                    <div class="total-footer">
                        ...
                        <span style="font-size: 24px;">{{ number_format($totalStockInGroup, fmod($totalStockInGroup, 1) == 0 ? 0 : 2, ',', '.') }}</span>
                        <span style="font-size: 14px; color: #64748b;">{{ strtoupper($firstProduct->unit) }}</span>
                    </div>
                </div>
                @endforeach
            </div>
...
<script>
    function switchWood(element, woodId) {
        ...
        // Update nama kayu di header
        const woodName = element.dataset.woodName || '';
        const headerName = document.getElementById('active-wood-name');
        if (headerName) {
            headerName.textContent = woodName ? 'KAYU ' + woodName : 'PABRIK KAYU JAYA ABADI';
        }
    ...
    // Set nama kayu default saat load (dari item aktif pertama)
    document.addEventListener('DOMContentLoaded', function() {
        const firstActive = document.querySelector('.wood-nav-item.active');
        if (firstActive) {
            const woodName = firstActive.dataset.woodName || '';
            const headerName = document.getElementById('active-wood-name');
            if (headerName && woodName) {
                headerName.textContent = 'KAYU ' + woodName;
            }
        }
    });
    ...
    function confirmDeleteHistory(url, type) {
        ...
        Modal.delete(itemName, function() {
            ...
            .then(response => {
                // Jika response redirect (bukan JSON), anggap berhasil
                if (response.redirected || response.status === 302) {
                    if (typeof Toast !== 'undefined') Toast.success('Berhasil dihapus');
            ...
            .then(result => {
                ...
                if (ok && data && data.success) {
                    if (typeof Toast !== 'undefined') Toast.success(data.message || 'Berhasil dihapus');
                    setTimeout(() => location.reload(), 1500);
                } else {
                    // Tampilkan pesan error spesifik dari server
                    const msg = (data && data.message) ? data.message : 'Gagal menghapus data.';
