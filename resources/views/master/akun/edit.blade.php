...
@section('breadcrumb', 'Master Data > Akun > Edit')
    ...
    <div class="card">
        ...
        <form action="{{ route('master.accounts.update', $account->id) }}" method="POST">
            ...
            <div class="card-body">
                ...
                <div class="form-section">
                    <div class="section-info">
                        ...
                        <p>Perbarui data akun sesuai Chart of Accounts.</p>
                    </div>
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <select name="type" 
                                ...
                                <option value="asset" {{ old('type', $account->type) == 'asset' ? 'selected' : '' }}>Asset (Aktiva)</option>
                                <option value="liability" {{ old('type', $account->type) == 'liability' ? 'selected' : '' }}>Liability (Kewajiban)</option>
                                <option value="equity" {{ old('type', $account->type) == 'equity' ? 'selected' : '' }}>Equity (Modal)</option>
                                <option value="revenue" {{ old('type', $account->type) == 'revenue' ? 'selected' : '' }}>Revenue (Pendapatan)</option>
                                <option value="cogs" {{ old('type', $account->type) == 'cogs' ? 'selected' : '' }}>COGS (Harga Pokok Penjualan)</option>
                                <option value="expense" {{ old('type', $account->type) == 'expense' ? 'selected' : '' }}>Expense (Beban)</option>
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" 
                                   ...
                                   class="form-control" 
                                   value="{{ old('opening_balance', $account->opening_balance > 0 ? number_format($account->opening_balance, 0, ',', '.') : '') }}" 
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" 
                                   ...
                                   class="form-control" 
                                   value="{{ old('opening_balance_date', $account->opening_balance_date ? $account->opening_balance_date->format('Y-m-d') : '') }}"
            ...
            <div class="card-footer">
                <button type="submit" class="btn-save">Perbarui Akun</button>
            </div>
        </form>
    </div>
</div>
