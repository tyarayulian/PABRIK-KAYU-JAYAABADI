...
<div class="page-container">
    <div class="card">
        ...
        <form action="{{ route('master.products.update', $product->id) }}" method="POST" onsubmit="return stripFormattingBeforeSubmit(this)">
            @csrf
            @method('PUT')
                ...
                <div class="form-section">
                    ...
                    <div class="form-grid" style="grid-template-columns: repeat(2, 1fr);">
                        <div class="form-group">
                            ...
                            <input type="text" name="wood_type" id="wood_type" class="form-control" value="{{ old('wood_type', $product->wood_type) }}" placeholder="Contoh: Duren, Senggon, Racuk">
                        </div>
                        <div class="form-group">
                            <label>Kategori Bentuk</label>
                            <input type="text" name="product_category" id="product_category" class="form-control" value="{{ old('product_category', $product->product_category) }}" placeholder="Contoh: Balok, Papan, Kasau" list="category_list">
                        ...
                        <div class="form-group">
                            <label>Ukuran</label>
                            <input type="text" name="size" id="size" class="form-control" value="{{ old('size', $product->size) }}" placeholder="Contoh: 8x12x4, 3x25x4">
                        </div>
                        <div class="form-group">
                            <label>Isi Per Kubik (Batang/Keping)</label>
                            <input type="text" inputmode="numeric" name="cubic_content" class="form-control" value="{{ old('cubic_content', $product->cubic_content) }}" placeholder="Contoh: 25">
                        </div>
                        <div class="form-group" style="grid-column: span 2;">
                            <label>Nama Produk Lengkap (Otomatis)*</label>
                            <input type="text" name="name" id="full_name" class="form-control" value="{{ old('name', $product->name) }}" placeholder="Akan terisi otomatis" required>
                        </div>
                        <div class="form-group">
                            <label>Satuan (Unit)*</label>
                            <input type="text" name="unit" class="form-control" value="{{ old('unit', $product->unit) }}" placeholder="Contoh: kubik, ikat, pcs" required>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-section" style="margin-bottom: 0;">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" name="cost" id="cost" class="form-control" value="{{ old('cost', (int)$product->cost) }}" placeholder="Rp 0" required>
                        </div>

                        <div class="form-group">
                            <label>Stok*</label>
                            <input type="text" inputmode="decimal" name="stock" class="form-control" value="{{ old('stock', (float)$product->stock) }}" placeholder="0" required>
                        </div>
                    </div>
                </div>
            </div>
