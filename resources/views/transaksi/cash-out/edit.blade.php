...
<div style="padding: 0 20px 40px 20px;">
    <div class="page-header">
        ...
        <div>
            ...
            <p style="font-size: 14px; color: #666; margin: 4px 0 0 0;">Perbarui data transaksi pengeluaran kas</p>
        </div>
    </div>
        ...
        <form action="{{ route('cash.out.update', $kasKeluar->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="date" value="{{ old('date', $kasKeluar->date->format('Y-m-d')) }}" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="category_id" required>
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('category_id', $kasKeluar->category_id) == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" required>{{ old('description', $kasKeluar->description) }}</textarea>
                    <small>Deskripsikan detail transaksi pengeluaran</small>
                </div>

                <div class="form-group">
                    <label>Jumlah (Rp)*</label>
                    <input type="number" name="amount" value="{{ old('amount', $kasKeluar->amount) }}" required min="1" step="0.01">
                ...
                <div class="form-group">
                    ...
                    @php
                        ...
                        $fileUrl = $hasFile ? asset('storage/' . $kasKeluar->file_path) : '';
                    ...
                    <div id="upload-container" class="upload-area {{ $hasFile ? 'has-file' : '' }}" onclick="document.getElementById('file_path').click()" style="border: 2px dashed #fee2e2; border-radius: 12px; padding: 20px; text-align: center; cursor: pointer; transition: all 0.2s; background: #fef2f2;">
                        <div id="upload-placeholder" class="upload-placeholder" style="display: {{ $hasFile ? 'none' : 'block' }}">
                            <i class="fas fa-cloud-upload-alt" style="font-size: 32px; color: #f87171; margin-bottom: 8px;"></i>
                        ...
                        <div id="file-preview-content" class="file-preview-content" style="display: block;">
                            <div class="preview-file-header" onclick="event.stopPropagation()" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                                <div class="preview-file-info" style="display: flex; align-items: center; gap: 10px;">
                                    <i class="fas fa-file-alt" style="color: #dc2626; font-size: 18px;"></i>
                                ...
                                <div class="preview-actions" style="display: flex; gap: 6px;">
                                    <button type="button" onclick="event.stopPropagation(); openViewer('{{ $fileUrl }}', '{{ $fileName }}')" style="background: #dc2626; color: white; border: none; padding: 6px 12px; border-radius: 6px; font-size: 12px; font-weight: 600; cursor: pointer;">LIHAT</button>
...
<script>
    document.getElementById('file_path').addEventListener('change', function(e) {
        ...
        if (e.target.files.length > 0) {
            label.innerHTML = `
                <i class="fas fa-check-circle" style="font-size: 24px; margin-bottom: 8px; color: #dc2626;"></i>
                <div style="color: #dc2626; font-weight: 600;">${e.target.files[0].name}</div>
