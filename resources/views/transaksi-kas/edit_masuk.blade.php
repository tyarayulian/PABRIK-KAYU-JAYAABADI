...
<style>
    ...
    .btn-save { background: #1e2a78; color: white; border: none; padding: 14px 32px; border-radius: 12px; font-weight: 700; font-size: 15px; font-family: 'Plus Jakarta Sans', sans-serif; cursor: pointer; transition: all 0.2s ease; }
    ...
    <div class="card">
        ...
        <form action="{{ route('cash.in.update', $kasMasuk->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card-body">
                ...
                <div class="form-section">
                    <div class="section-info">
                        ...
                        <p>Sesuaikan detail transaksi pemasukan kas.</p>
                    </div>
                    <div class="form-grid">
                        <div class="form-group">
                            <label>Tanggal*</label>
                            <input type="date" name="date" value="{{ old('date', $kasMasuk->date->format('Y-m-d')) }}" class="form-control" required>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="category_id" class="form-control" required>
                                ...
                                @foreach($categories as $cat)
                                    <option value="{{ $cat->id }}" {{ old('category_id', $kasMasuk->category_id) == $cat->id ? 'selected' : '' }}>
                                        {{ $cat->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group full-width">
                            ...
                            <input type="text" id="amount" name="amount" placeholder="Rp 0" value="{{ old('amount', (int)$kasMasuk->amount) }}" class="form-control" required>
                        </div>

                        <div class="form-group full-width">
                            <label>Keterangan / Deskripsi</label>
                            <textarea name="description" placeholder="Contoh: Penerimaan pembayaran dari pelanggan" class="form-control" rows="3" style="resize: none;">{{ old('description', $kasMasuk->description) }}</textarea>
                        </div>
                            ...
                            <div id="file-preview-content" class="file-preview-content" style="display: {{ $hasFile ? 'block' : 'none' }}">
                                <div class="preview-file-header" onclick="event.stopPropagation()">
                                    ...
                                    <div class="preview-actions">
                                        ...
                                        <a id="btn-download-file" href="{{ $fileUrl }}" download="{{ $fileName }}" class="preview-btn-sm"><i class="fas fa-download"></i></a>
            ...
            <div class="card-footer">
                <button type="submit" class="btn-save">Update Pemasukan</button>
            </div>
        </form>
    </div>
</div>
